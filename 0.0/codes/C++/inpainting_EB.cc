...
void keb2g(fltarray &Kappa, double &Field1, double &Field2, fltarray &Gamma)
{
  ...
  for (i=0; i<n1; i++)
    ...
    for (j=0; j<n2; j++)
      ...
      if (l1*l1+l2*l2 == 0)
        {
	  PsiE_tilde(j, i).real()=0.;
	  PsiE_tilde(j, i).imag()=0.;
	  PsiB_tilde(j, i).real()=0.;
	  PsiB_tilde(j, i).imag()=0.;
	}
      else
	{
	  PsiE_tilde(j,i).real()=-(2./double(l1*l1+l2*l2))*KappaE_tilde(j,i).real();      
	  PsiE_tilde(j,i).imag()=-(2./double(l1*l1+l2*l2))*KappaE_tilde(j,i).imag();   
	  PsiB_tilde(j,i).real()=-(2./double(l1*l1+l2*l2))*KappaB_tilde(j,i).real();      
	  PsiB_tilde(j,i).imag()=-(2./double(l1*l1+l2*l2))*KappaB_tilde(j,i).imag();   
        }
      }
    }
  for (i=0; i<n1; i++)
    ...
    for (j=0; j<n2; j++)
      {
        ...
        GammaE1_tilde(j,i).real() =-double(.5*(l1*l1-l2*l2))*PsiE_tilde(j,i).real();
	GammaE1_tilde(j,i).imag() =-double(.5*(l1*l1-l2*l2))*PsiE_tilde(j,i).imag();
        GammaE2_tilde(j,i).real() =-double(l1*l2)*PsiE_tilde(j,i).real();
        GammaE2_tilde(j,i).imag() =-double(l1*l2)*PsiE_tilde(j,i).imag();
        
        GammaB1_tilde(j,i).real() =-double(.5*(l1*l1-l2*l2))*PsiB_tilde(j,i).real();
        GammaB1_tilde(j,i).imag() =-double(.5*(l1*l1-l2*l2))*PsiB_tilde(j,i).imag();
        GammaB2_tilde(j,i).real() =-double(l1*l2)*PsiB_tilde(j,i).real();
        GammaB2_tilde(j,i).imag() =-double(l1*l2)*PsiB_tilde(j,i).imag();
      }
    }
  Reverse = True;
...
void g2keb(fltarray &Gamma, double &Field1, double &Field2, fltarray &Kappa)
{
  ...
  for (i=0; i<n1; i++)
    ...
    for (j=0; j<n2; j++)
      ...
      if (l1*l1+l2*l2 == 0)
        {
	  KappaE_tilde(j, i).real()=0.;
	  KappaE_tilde(j, i).imag()=0.;
	  KappaB_tilde(j, i).real()=0.;
	  KappaB_tilde(j, i).imag()=0.;
	}
      else
	{
          KappaE_tilde(j,i).real()=(double(l1*l1-l2*l2)*Gamma1E_tilde(j,i).real()+double(2.*l1*l2)*Gamma2E_tilde(j,i).real())/double(l1*l1+l2*l2);
          KappaE_tilde(j,i).imag()=(double(l1*l1-l2*l2)*Gamma1E_tilde(j,i).imag()+double(2.*l1*l2)*Gamma2E_tilde(j,i).imag())/double(l1*l1+l2*l2);
          KappaB_tilde(j,i).real()=(double(l1*l1-l2*l2)*Gamma1B_tilde(j,i).real()+double(2.*l1*l2)*Gamma2B_tilde(j,i).real())/double(l1*l1+l2*l2);
          KappaB_tilde(j,i).imag()=(double(l1*l1-l2*l2)*Gamma1B_tilde(j,i).imag()+double(2.*l1*l2)*Gamma2B_tilde(j,i).imag())/double(l1*l1+l2*l2);
        }
      }
    }
